...
namespace rig {
enum class TouchKind : uint8_t { None, PressStarted, Moved, DragStarted, DragMoved, SwipeUp, SwipeDown, SwipeLeft, SwipeRight, Tap, HoldStarted, Released, Cancelled };
...
struct GestureThresholds { uint8_t tapMovement=14,dragDistance=18; uint16_t categoryHoldMs=1200,centerHoldMs=1200,swipeVelocity=180; };
class GestureRecognizer {
 public:
  ...
  TouchEvent sample(bool down,bool valid,uint16_t x,uint16_t y,uint32_t now,TouchZone zone=TouchZone::Outside) {
    ...
    if (down && !active_) { active_=true; dragging_=held_=false; zone_=zone; sx_=px_=velocityX_=x; sy_=py_=velocityY_=y; started_=last_=velocityAt_=now; return make(TouchKind::PressStarted,x,y,now); }
    if (down && active_) {
      const int dx=int(x)-int(sx_),dy=int(y)-int(sy_); const uint32_t velocityDt=now-velocityAt_;
      if(velocityDt>80){vx_=vy_=0;velocityX_=x;velocityY_=y;velocityAt_=now;}
      else if(velocityDt>=40){vx_=int16_t((int(x)-int(velocityX_))*1000/int(velocityDt));vy_=int16_t((int(y)-int(velocityY_))*1000/int(velocityDt));velocityX_=x;velocityY_=y;velocityAt_=now;}
      ...
      else if (x!=px_||y!=py_) kind=TouchKind::Moved;
 ...
 private:
  ...
  GestureThresholds thresholds_; bool active_=false,dragging_=false,held_=false; TouchZone zone_=TouchZone::Outside; uint16_t sx_=0,sy_=0,px_=0,py_=0,velocityX_=0,velocityY_=0;int16_t vx_=0,vy_=0;uint32_t started_=0,last_=0,velocityAt_=0;
};
}
