...
void RigUi::handleTouch(const TouchEvent &e) {
  ...
  if (e.kind == TouchKind::PressStarted) {
    ...
    gestures_.begin(stableLayer_, static_cast<uint8_t>(page_), e.x, e.y,
                    e.timestampMs, scroll_.offset(), e.y >= 24 && e.y <= 52, row);
  ...
  if (e.kind == TouchKind::HoldStarted && stableLayer_ == StableLayer::Category &&
      gestures_.active() && gestures_.session().startY < 55) {
    gestures_.cancel(); contact_ = false; expand(); return;
  }
  if (e.zone == TouchZone::Center && e.kind == TouchKind::HoldStarted) {
    gestures_.cancel(); selection_.cancel(); contact_ = false; showStatus(); return;
  }
...
void RigUi::expand(){stableLayer_=StableLayer::Menu;mode_=UiMode::Expanding;animationAtUs_=micros();transitionProgress_=0;int count=rowCount();scroll_.configure(166,count*30);selection_.setHighlighted(scroll_.rowAt(120,58,30,count));invalidation_.add(BackgroundDirty|PanelViewportDirty);}
