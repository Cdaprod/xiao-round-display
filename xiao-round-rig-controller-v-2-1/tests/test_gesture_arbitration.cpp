...
int main() {
  assert(GestureArbitrator::categoryOpenControl(120,48));
  assert(!GestureArbitrator::categoryOpenControl(120,47));
  { auto g=category(); assert(g.move(135,120,0,0,10)==GestureOwner::None); assert(g.release(135,120,0,0,-1)==GestureResolution::None); }
  { GestureArbitrator g; g.begin(StableLayer::Category,2,120,120,0,0,true,-1); assert(g.release(134,120,0,0,-1)==GestureResolution::OpenMenu); }
  ...
  { auto g=category(); assert(g.move(155,140,0,0,10)==GestureOwner::CategoryHorizontal);assert(g.release(155,140,0,0,-1)==GestureResolution::CategoryPrevious); }
  ...
  { auto g=category(); g.move(150,120,0,0,10); assert(g.release(150,80,0,-500,-1)==GestureResolution::CategoryPrevious); }
  ...
  { auto g=category(); g.move(150,120,100,0,10); assert(g.release(150,120,100,0,-1)==GestureResolution::CategoryPrevious); assert(g.session().categoryAtDown==2); }
  { auto g=category(); g.move(140,120,300,0,50); assert(g.release(140,120,300,0,-1)==GestureResolution::CategoryPrevious); }
  ...
  { auto g=menu(0,true); g.move(120,90,0,500,10); assert(g.release(120,90,0,500,-1)==GestureResolution::KeepMenu); assert(g.session().categoryAtDown==2); }
