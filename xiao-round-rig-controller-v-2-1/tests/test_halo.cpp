...
int main(){
 ...
 model.setExpanded(true,300000);assert(model.presentation()==HaloPresentation::Exiting);model.touchRelease(310000);assert(model.presentation()==HaloPresentation::Exiting);model.update(480000);assert(model.needsFrame(480000));model.framePresented(480000);assert(model.presentation()==HaloPresentation::Hidden);assert(!model.needsFrame(500000,true));
